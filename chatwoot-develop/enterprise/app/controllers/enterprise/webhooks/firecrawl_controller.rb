class Enterprise::Webhooks::FirecrawlController < ActionController::API
  ...
  def process_payload
    AI Agent::Tools::FirecrawlParserJob.perform_later(assistant_id: assistant.id, payload: payload) if crawl_page_event?
  ...
  def assistant
    @assistant ||= AI Agent::Assistant.find(permitted_params[:assistant_id])
  end
