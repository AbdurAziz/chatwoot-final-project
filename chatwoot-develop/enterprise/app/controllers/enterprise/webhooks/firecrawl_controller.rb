class Enterprise::Webhooks::FirecrawlController < ActionController::API
  ...
  def process_payload
    AI Agent::Tools::FirecrawlParserJob.perform_later(topic_id: topic.id, payload: payload) if crawl_page_event?
  ...
  def topic
    @topic ||= AI Agent::Topic.find(permitted_params[:topic_id])
  end
