...
RSpec.describe AI Agent::Documents::ResponseBuilderJob, type: :job do
  let(:assistant) { create(:aiagent_assistant) }
  let(:document) { create(:aiagent_document, assistant: assistant) }
  let(:faq_generator) { instance_double(AI Agent::Llm::FaqGeneratorService) }
  let(:faqs) do
    [
  ...
  before do
    allow(AI Agent::Llm::FaqGeneratorService).to receive(:new)
  ...
  describe '#perform' do
    context 'when processing a document' do
      ...
      it 'creates new responses for each FAQ' do
        expect do
          described_class.new.perform(document)
        end.to change(AI Agent::AssistantResponse, :count).by(2)
