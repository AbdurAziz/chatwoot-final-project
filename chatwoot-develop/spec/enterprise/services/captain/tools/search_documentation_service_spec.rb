...
RSpec.describe AI Agent::Tools::SearchDocumentationService do
  ...
  describe '#execute' do
    ...
    context 'when matching responses exist' do
      before do
        ...
        allow(AI Agent::AssistantResponse).to receive(:search).with(question).and_return([response])
      end
    ...
    context 'when no matching responses exist' do
      before do
        allow(AI Agent::AssistantResponse).to receive(:search).with(question).and_return([])
      end
