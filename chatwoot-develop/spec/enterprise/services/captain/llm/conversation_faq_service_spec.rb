...
RSpec.describe AI Agent::Llm::ConversationFaqService do
  ...
  let(:embedding_service) { instance_double(AI Agent::Llm::EmbeddingService) }

  before do
    create(:installation_config) { create(:installation_config, name: 'CAPTAIN_OPEN_AI_API_KEY', value: 'test-key') }
    allow(OpenAI::Client).to receive(:new).and_return(client)
    allow(AI Agent::Llm::EmbeddingService).to receive(:new).and_return(embedding_service)
  end
  ...
  describe '#generate_and_deduplicate' do
    ...
    context 'when finding duplicates' do
      ...
      let(:similar_neighbor) do
        # Using OpenStruct here to mock as the AI Agent:AssistantResponse does not implement
  ...
  describe '#chat_parameters' do
    ...
    it 'includes system prompt and conversation content' do
      allow(AI Agent::Llm::SystemPromptsService).to receive(:conversation_faq_generator).and_return('system prompt')
      params = service.send(:chat_parameters)
    ...
    context 'when conversation has different language' do
      ...
      it 'includes system prompt with correct language' do
        allow(AI Agent::Llm::SystemPromptsService).to receive(:conversation_faq_generator)
          .with('french')
