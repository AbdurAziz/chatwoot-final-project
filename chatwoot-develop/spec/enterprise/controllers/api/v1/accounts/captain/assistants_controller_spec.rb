...
RSpec.describe 'Api::V1::Accounts::AI Agent::Assistants', type: :request do
      ...
      it 'creates a new assistant' do
        expect do
          post "/api/v1/accounts/#{account.id}/aiagent/assistants",
               ...
               as: :json
        end.to change(AI Agent::Assistant, :count).by(1)
  ...
  describe 'POST /api/v1/accounts/{account.id}/aiagent/assistants/{id}/playground' do
    ...
    context 'when it is an agent' do
      it 'generates a response' do
        chat_service = instance_double(AI Agent::Llm::AssistantChatService)
        allow(AI Agent::Llm::AssistantChatService).to receive(:new).with(assistant: assistant).and_return(chat_service)
